...
function login($usuario, $password){
	...
	$stmt =$mysqli->prepare("SELECT id_persona, nombre_persona, edad_persona, tdoc_persona, doc_persona, tel_persona, email_persona, usuario_persona,contrasena_persona, id_rol FROM persona WHERE usuario_persona = ? || email_persona = ? LIMIT 1");
	...
	$stmt->bind_param("sss", $usuario,$usuario,$edad_persona);
	$stmt->execute();
	$stmt->store_result();
	$rows = $stmt->num_rows;
	...
	if ($rows > 0 ) {
		if (isActivo($usuario)) {
			$stmt->bind_result($idUsuario,  $nombre_persona,$passwd,$id_rol);
			$stmt->fetch();
